...
MainTab:AddSwitch("Anti AFK", function(state)
    if state then
        ...
        uselesslabelone.Text = "Anti Afk BY Karma"
...
local TimeLabel = MusicTab:AddLabel("Ã¢ÂÂ±Ã¯Â¸Â 00:00 / 00:00")
...
task.spawn(function()
	while task.wait(0.1) do
		if currentSound and currentSound:IsDescendantOf(SoundService) and currentSound.IsLoaded then
			TimeLabel.Text = "Ã¢ÂÂ±Ã¯Â¸Â " .. formatTime(currentSound.TimePosition) .. " / " .. formatTime(currentSound.TimeLength)
...
local function calculateHitsToKill(health, damage)
    if damage <= 0 then return "Ã¢Ë†Å¾" end
    local hits = math.ceil(health / damage)
    return hits > 50 and "Ã¢Ë†Å¾" or (hits < 1 and 1 or hits)
end
...
Credits:AddLabel("paid version Rework")
Credits:AddLabel("Script Made by username: CHIKIKAR4)
Credits:AddLabel("Scrip by: KARMA")
Credits:AddLabel("I hope you like the script, for bugs or suggestions on my Tiktok")
Credits:AddLabel("Espero que les guste el script, para bugs o sugerencias en mi Tiktok")
Credits:AddLabel("Tiktok: karma098")
Credits:AddButton("Copy Link Direc", function()
setclipboard("FZK ON TOP")
        end)
